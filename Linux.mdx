...
- **info**: 列出和系統相關的資訊，如映像檔數、Container 數、檔案系統目錄、Linux 核心版本，使用 Linux 版本、CPU 及記憶體等。
- **docker version**: 列出目前 Docker 的版本
...
- **unshare**: 創造新的 namespace
  **options**
  ...
  - **-\-mount-proc**: 將新的 namepace 中 mount 一個 /proc 檔案系統 (/proc 為一個虛擬檔案系統，提供關於系統核心及正在執行的 process 資訊)
- **mount**: 掛載文件系統的命令
  **options**
  - **-t type**: 指定文件系統類型
  - **-o options**: 指定掛載選項
...
- **rm**: 刪除檔案
  **options**
  ...
  - **-r**: 刪除整個資料夾
  - **-f**: 強制刪除
  - **fileName\***: 表示從 fileName 後的檔案都要刪除
...
- **find**: 搜尋指定的檔案
  **options**
  - **-iname**: 不在乎大小寫搜尋
  - **-name**: 要求大小寫相同
  - **-size**: 按檔案大小查找，表示大於(使用+)或小於(使用-)指定大小，單位可以是 c（位元組）、w（字數）、b（區塊數）、k（KB）、M（MB）或 G（GB）
...
- **df**: 用來顯示檔案系統的磁盤使用情況的工具
  **options**
  - **-h or -\-human-readable**: 將磁盤大小以人類可讀的方式（例如 GB、MB）顯示。
...
- **gzip fileName**: 壓縮檔案(副檔名為 .gz)
  **options**
  ...
  - **-q or -\-quite**: 不顯示警告訊息
  - **-v or -\-verbose**: 顯示壓縮比
- **gunzip fileName**: 解開被 gzip 壓縮過的文件
  **options**
...
- **tar**: 用於備份檔案
  **options**
  ...
  - **-z or -\-gzip or -\-ungzip**: 透過 gzip 指令處理備份檔
...
- **ln file1 file2**: 創建檔案連結
  **重要觀念**
  - **硬連結**: 將兩個檔案指向同一個 inode(檔案中的唯一識別符號)，兩個檔案存在同一個檔案系統中，原始檔案被刪除後資料依然存在，只要有一個硬連接指向這個 inode 資料就不會被刪除
...
- **SIGKILL(立即中止)**: 編號 9，強制立即中止，資料容易有丟失風險
...
**SIGTSTP**: 暫停訊號，例如按下 Ctrl + Z，正在跑的 Process 會被暫停並且被 zsh 放到背景
...
- **加減乘除**: `+-*/`
- **取餘數**: `%
- **var++ var--**: 指定變數後將變數加或減 1
- **++var --var**: 將變數加或減 1 後指定變數
- **位元/邏輯反轉**: `!` or `~`
- **次方**:`**`
- **邏輯運算**
  - AND: `&&`
...
> 用於結束當前的 script，可指定一個數字作為退出碼。退出碼通常用於指示上一個命令的執行狀態，0 表示成功，非零表示錯誤，可用來做一些邏輯判斷或錯誤處理
