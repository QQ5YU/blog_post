...
- **-q or -\-quiet**: 只列出映像檔 ID
- **-\-format goLen 樣板語法**: 指定列出格式，ex. docker image ls --format "{{.ID}}: {{.Repository}}"
- **-\-digests**: 顯示 image digest (摘要)

### docker pull Registry 的位址/imageName:tag

> 從 Docker Hub 下載映像檔(沒有 tag 的話預設會拉最新版)，Registry 的位址預設為官方 registry
...
**一個映像檔可以有很多不同的 tag name**

> 將 latest 和 u14 兩個 TAG 指到同一個映像檔 ID
...
> 想要和其它人交換時 ，不想上傳到 Docker Hub，不想自己架設私有 Docker Registry，就可以用這兩個參數存成 tarball 格式及匯出。
...
> 讓 container 暫停直到 container s 停止為止
...
- **-d or -id**:離開 container 時該 container 停止
- **-td**: 離開 container 時該 container 在背景執行
...
- **sshcmd**: 共享的機器 username@IP(Host3):Host3 路徑
- **password**: 共享機器密碼
- **sshvolume**: 為 volume 命名
...
> 檢視 images、containers、volumes 所占用的空間
...
- **-b bridge or -\-bridge=bridge**: 指定容器掛載的 bridge
...
- \*_-\-iptables=true|false_: 是否允許 docker 新增 iptables 規則，iptables 為 Linux 上的防火牆軟體
...
> 虛懸映像 (dangling image)，由`docker build` 或 `docker pull`產生，指的是 repositoryName、tag 都顯示為 `<none>` 的 image，常見於官方發布新版本後，原先的映像檔使用新版本的映像，而導致舊版本沒被使用，這樣會占空間，透過`docker images` 會顯示。
